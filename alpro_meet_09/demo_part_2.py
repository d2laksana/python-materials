...
def showPesanan():
    ...
    for makan in pesanan_makanan:
        print("%s. %s (%s) : %s" %
              (indeks, makan["nama"], makan["jumlah"], makan["harga"]))
        indeks += 1
    for minum in pesanan_minuman:
        print("%s. %s (%s) : %s" %
              (indeks, minum["nama"], minum["jumlah"], minum["harga"]))
        indeks += 1
...
while True:
    ...
    if pilih_menu == '1':
        ...
        while (orderMakananLagi == 'y'):
            ...
            jml_porsi = int(input("Jumlah porsi %s: " %
                            makanan[no_menu-1]["nama"]))
            ...
            print("\n%s porsi %s dicatat." %
                  (jml_porsi, makanan[no_menu-1]["nama"]))
        ...
        while (orderMinumLagi == 'y'):
            ...
            print("\n%s porsi %s dicatat." %
                  (jml_porsi, minuman[no_menu-1]["nama"]))
        ...
        for x in pesanan_makanan:
            ...
            print(x["nama"], x["jumlah"], "=", subtotal)

            total = total + subtotal

        # Total MInuman
        for x in pesanan_minuman:
            subtotal = x["harga"] * x["jumlah"]
            print(x["nama"], x["jumlah"], "=", subtotal)
        ...
        print("\nTotal tagihan :", total)
        uang_bayar = int(input("Uang yang Anda bayar : "))
        print("______________________________________")
        print("Kembalian :", uang_bayar - total)
